...
function exec!(w::Automaton, action::Union{Symbol, Nothing} = nothing, context::Any = nothing)
    w.state = exec(w, w.state, action, context)
end

exec(w::Automaton, context::Any = nothing)                  = exec(w, w.state, nothing, context)
exec(w::Automaton, action::Symbol, context::Any = nothing)  = exec(w, w.state, action,  context)
function exec(w::Automaton, s::State, action::Union{Symbol, Nothing}, context::Any = nothing) :: State
    @assert s in w.states "State not available in the automaton ($s)"

    prev = s
    n = _exec(w, prev, action, context)
    if MULTISTEP
        while(prev != n)
            prev = n
            n = _exec(w, prev, nothing, context)
        end
    end
